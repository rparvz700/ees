...
<div class="content">
    <div class="block block-rounded">
        <div class="block-header block-header-default">
            <h3 class="block-title">Add Questions</h3>
        </div>
        <div class="block-content fs-sm data-content">
            <form class="mb-4" action="{{ route('questions.save') }}" method="POST" autocomplete="off">
                @csrf
                ...
                <div id="questions-container">
                    ...
                    @forelse(old('questions', $questions->toArray()) as $index => $q)
                        <div class="question-item border rounded p-3 mb-3">
                            <h3 class="question-number-label mb-3">Question {{ $index+1 }}</h3>
                            <input type="hidden" name="questions[{{ $index }}][number]" 
                                   value="{{ $q['number'] ?? $index+1 }}" class="question-number-input">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 col-sm-12 mb-4">
                                    ...
                                    <select class="js-select2 form-select"
                                            name="questions[{{ $index }}][dimension][]"
                                            multiple="multiple"
                                            data-placeholder="Select Dimensions"
                                            required>
                                        @php
                                            $selected = $q['dimension'] ?? [];
                                        ...
                                        @foreach($allDimensions as $dim)
                                            <option value="{{ $dim }}" 
                                                {{ in_array($dim, $selected) ? 'selected' : '' }}>
                                                {{ $dim }}
                                            </option>
                                        @endforeach
                                    </select>
                                    @error("questions.$index.dimension")<div class="text-danger"><small>{{ $message }}</small></div>@enderror
                                </div>

                                <!-- Reverse coded -->
                                <div class="col-md-4 col-sm-12 mb-4">
                                    <label class="form-label">Reverse Coded?</label>
                                    <select class="form-select" name="questions[{{ $index }}][reverse_coded]">
                                        <option value="0" {{ ($q['reverse_coded'] ?? 0) == 0 ? 'selected' : '' }}>No</option>
                                        <option value="1" {{ ($q['reverse_coded'] ?? 0) == 1 ? 'selected' : '' }}>Yes</option>
                                    </select>
                                </div>
                                ...
                                <div class="col-12 mb-4">
                                    ...
                                    <textarea class="form-control" 
                                              ...
                                              rows="3" required>{{ $q['text'] ?? '' }}</textarea>
                                    @error("questions.$index.text")<div class="text-danger"><small>{{ $message }}</small></div>@enderror
                                </div>
                    ...
                    @empty
                        {{-- If no questions, render one blank --}}
                        <div class="question-item border rounded p-3 mb-3">
                            ...
                            <div class="row">
                                <div class="col-md-6 col-sm-12 mb-4">
                                    <label class="form-label">Dimension<span class="text-danger">*</span></label>
                                    <select class="js-select2 form-select" name="questions[0][dimension][]" multiple="multiple" required>
                                        @foreach($allDimensions as $dim)
...
<script>
let questionIndex = {{ count(old('questions', $questions)) ?: 1 }};
